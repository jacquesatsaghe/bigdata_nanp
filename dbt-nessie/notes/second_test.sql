...
-- WITH (location = 's3://marts-bucket/warehouse/dummy_schema/'); is where your data will reside on minio
-- create bucket 'marts-bucket' before
...
-- CREATE SCHEMA dbt.dummy_schema 
-- WITH (location = 's3://marts-bucket/warehouse/dummy_schema/');
CREATE SCHEMA dbt.dummy_schema;

-- 2. Création d'une table optimisée pour vos petits fichiers
CREATE TABLE dbt.dummy_schema.sensor_data (
    sensor_id VARCHAR,
    value DOUBLE,
    ts TIMESTAMP(3)
)
